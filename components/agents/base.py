...
from components.utils.format import get_ddp_model_weights, DiscreteSupport, symexp
from components.utils.loss import (
    kl_loss,
...
from components.data.trajectory import GameTrajectory
from components.data.augmentation import Transforms
...
class Agent:
    def __init__(self, config):
        ...
        self._update = False
        self.use_ddp = (
            True if config.ddp.world_size > 1 or config.ddp.training_size > 1 else False
        )
    ...
    def train(self, rank, replay_buffer, storage, batch_storage, logger):
        ...
        while not (
            ray.get(replay_buffer.get_transition_num.remote())
            >= self.config.train.start_transitions
        ):
            time.sleep(1)
            pass
        ...
        # while loop
        self_play_reteurn = 0.0
        traj_num, transition_num = 0, 0
        eval_score, eval_best_score = 0.0, 0.0
        prev_eval_counter = -1
        eval_counter = 0
        ...
        while not self.is_finished(step_count):
            ...
            if (
                is_main_process
                and step_count % self.config.train.self_play_update_interval == 0
            ):
            ...
            if (
                is_main_process
                and step_count % self.config.train.reanalyze_update_interval == 0
            ):
            ...
            if (
                is_main_process
                and step_count % self.config.train.save_ckpt_interval == 0
            ):
            ...
            if is_main_process and step_count % pb_interval == 0:
                left_steps = (
                    self.config.train.training_steps
                    + self.config.train.offline_training_steps
                    - step_count
                )
                left_time = (left_steps * avg_time) / 3600
                batch_queue_size = batch_storage.get_len()
                train_log_str = (
                    "[Train] {}, step {}/{}, {:.3f}h left. lr={:.3f}, avg time={:.3f}s, batchQ={}, "
                    ...
                    "".format(
                        ...
                        loss_data["loss/value"],
                        loss_data["loss/policy"],
                        loss_data["loss/consistency"],
                        loss_data["loss/entropy"],
                    )
                )
            ...
            if is_main_process and step_count % self.config.log.log_interval == 0:
                ...
                eval_scalar, remote_scalar, remote_distribution = ray.get(
                    storage.get_log.remote()
                )
                ...
                if len(eval_scalar) > 0:
                    ...
                    min_score, max_score = (
                        eval_scalar["eval/min_score"],
                        eval_scalar["eval/max_score"],
                    )
                    eval_counter, eval_best_score = ray.get(
                        [
                            storage.get_eval_counter.remote(),
                            storage.get_best_score.remote(),
                        ]
                    )

                    eval_log_str = (
                        "Eval {} at at step {}, score = {:.3f}(min: {:.3f}, max: {:.3f}), "
                ...
                traj_num, transition_num, total_priorities = ray.get(
                    [
                        replay_buffer.get_traj_num.remote(),
                        replay_buffer.get_transition_num.remote(),
    ...
    def periodic_reset_model(self, model):
        self.reset_network(model.value_policy_model.fc_backbone)
    ...
    def update_weights(
        ...
        gradient_scale = 1.0 / unroll_steps
        ...
        obs_batch_raw = torch.from_numpy(obs_batch_ori.copy()).cuda().float()

        obs_batch = obs_batch_raw[:, 0:n_stack]  # obs_batch: current observation
        obs_target_batch = obs_batch_raw[
            :, n_stack:
        ]  # obs_target_batch: observation of next steps
        ...
        if self.config.env.env in ["DMC", "Gym"]:
            action_batch = torch.from_numpy(action_batch.copy()).float().cuda()
        else:
            action_batch = (
                torch.from_numpy(action_batch.copy()).cuda().unsqueeze(-1).long()
            )
        mask_batch = torch.from_numpy(mask_batch.copy()).cuda().float()
        weights = torch.from_numpy(weights_lst.copy()).cuda().float()

        max_value_target = np.array([target_values, search_values]).max(0)

        target_value_prefixes = (
            torch.from_numpy(target_value_prefixes.copy()).cuda().float()
        )
        target_values = torch.from_numpy(target_values.copy()).cuda().float()
        target_actions = torch.from_numpy(target_actions.copy()).cuda().float()
        target_policies = torch.from_numpy(target_policies.copy()).cuda().float()
        target_best_actions = (
            torch.from_numpy(target_best_actions.copy()).cuda().float()
        )
        top_value_masks = torch.from_numpy(top_value_masks.copy()).cuda().float()
        mismatch_masks = torch.from_numpy(mismatch_masks.copy()).cuda().float()
        search_values = torch.from_numpy(search_values.copy()).cuda().float()
        max_value_target = torch.from_numpy(max_value_target.copy()).cuda().float()

        # transform value and reward to support
        target_value_prefixes_support = DiscreteSupport.scalar_to_vector(
            target_value_prefixes, **self.config.model.reward_support
        )

        with autocast(device_type="cuda"):
            if obs_batch.shape[1] == 1:
                obs_batch = obs_batch.squeeze(1)
        ...
        if self.config.model.value_support.type == "symlog":
            ...
            scaled_value = DiscreteSupport.vector_to_scalar(
        ...
        if self.config.train.value_target == "sarsa":
            ...
            if step_count < self.config.train.start_use_mix_training_steps:
                ...
                this_target_values = target_values * top_value_masks.unsqueeze(
                    1
                ).repeat(1, unroll_steps + 1) + search_values * (
                    1 - top_value_masks
                ).unsqueeze(1).repeat(1, unroll_steps + 1)
        ...
        # update priority
        fresh_priority = (
            L1Loss(reduction="none")(scaled_value.squeeze(-1), this_target_values[:, 0])
            .detach()
            .cpu()
            .numpy()
        )
        ...
        if self.config.env.env in ["DMC", "Gym"]:
            ...
            sigma = (
                policies[:, policies.shape[-1] // 2 :].detach().cpu().numpy().flatten()
            )
            ...
            for step_i in range(unroll_steps):
                ...
                states, value_prefixes, values, policies, reward_hidden = (
                    model.recurrent_inference(
                        states, action_batch[:, step_i], reward_hidden, training=True
                    )
                )

                beg_index = step_i
                end_index = step_i + n_stack

                if n_stack == 1:
                    obs_target_batch_i = obs_target_batch[:, beg_index]
                else:
                    obs_target_batch_i = obs_target_batch[:, beg_index:end_index]
                # consistency loss
                gt_next_states = model.do_representation(obs_target_batch_i)
                ...
                consistency_loss += (
                    cosine_similarity_loss(dynamic_states_proj, gt_states_proj) * mask
                )

                # reward, value, policy loss
                if self.config.model.reward_support.type == "symlog":
                    value_prefix_loss += (
                        symlog_loss(value_prefixes, target_value_prefixes[:, step_i])
                        * mask
                    )
                else:
                    value_prefix_loss += (
                        kl_loss(
                            value_prefixes, target_value_prefixes_support[:, step_i]
                        )
                        * mask
                    )

                value_loss += (
                    Value_loss(values, this_target_values[:, step_i + 1], self.config)
                    * mask
                )
                ...
                if self.config.env.env in ["DMC", "Gym"]:
                    policy_loss_i, entropy_loss_i = continuous_loss(
                        policies,
                        target_actions[:, step_i + 1],
                        target_policies[:, step_i + 1],
                    ...
                    policy_loss_i = (
                        kl_loss(policies, target_policies[:, step_i + 1]) * mask
                    )
                    policy_loss += policy_loss_i
                ...
                # reset reward hidden
                if (
                    self.config.model.value_prefix
                    and (step_i + 1) % self.config.model.lstm_horizon_len == 0
                ):
                    reward_hidden = self.init_reward_hidden(batch_size)

        # total loss
        loss = (
            value_prefix_loss * self.config.train.reward_loss_coeff
        ...
        loss_data.update(
            {
                ...
                "loss/weighted": weighted_loss.mean().item(),
                "loss/consistency": consistency_loss.mean().item(),
                "loss/value_prefix": value_prefix_loss.mean().item(),
                "loss/value": value_loss.mean().item(),
        ...
        other_scalar.update(
            {
                "other_log/target_value_prefix_max": target_value_prefixes.detach()
                .cpu()
                .numpy()
                .max(),
                "other_log/target_value_prefix_min": target_value_prefixes.detach()
                .cpu()
                .numpy()
                .min(),
                "other_log/target_value_prefix_mean": target_value_prefixes.detach()
                .cpu()
                .numpy()
                .mean(),
                "other_log/target_value_mean": target_values.detach()
                .cpu()
                .numpy()
                .mean(),
                "other_log/target_value_max": target_values.detach()
                .cpu()
                .numpy()
                .max(),
                "other_log/target_value_min": target_values.detach()
                .cpu()
                .numpy()
                .min(),
                "other_log/mismatch_num": batch_size * (unroll_steps + 1)
                - mismatch_masks.sum().detach().cpu().numpy(),
            }
        )
    ...
    def update_augmentation_transform(self):
        if self.config.augmentation:
            raise NotImplementedError(
                "Please implement the update_augmentation_transform method in your agent."
            )
            self.transforms = Transforms(
                self.config.augmentation,
                image_shape=(self.obs_shape[1], self.obs_shape[2]),
            )
...
def train_ddp(agent, rank, replay_buffer, storage, batch_storage, logger):
    ...
    if agent.use_ddp:
        DDP_setup(
            rank=rank,
            world_size=agent.config.ddp.world_size,
            training_size=agent.config.ddp.training_size,
            address="127.0.0.1",
        )
        model = DDP(model, device_ids=[rank])
    ...
    while not (
        ray.get(replay_buffer.get_transition_num.remote())
        >= agent.config.train.start_transitions
    ):
        time.sleep(1)
        pass
    print("[Train] Begin training...")
    ...
    total_time = 0
    total_steps = (
        agent.config.train.training_steps + agent.config.train.offline_training_steps
    )
    if is_main_process:
        pb = tqdm(np.arange(total_steps), leave=True)
    ...
    while not agent.is_finished(step_count):
        ...
        if (
            is_main_process
            and step_count % agent.config.train.self_play_update_interval == 0
        ):
        ...
        if (
            is_main_process
            and step_count % agent.config.train.reanalyze_update_interval == 0
        ):
            storage.set_weights.remote(recent_weights, "reanalyze")
            target_model.set_weights(recent_weights)
            target_model.cuda()
        ...
        scalers, log_data = agent.update_weights(
            model.module,
            batch,
            optimizer,
            replay_buffer,
        ...
        if is_main_process and step_count % pb_interval == 0:
            left_steps = (
                agent.config.train.training_steps
                + agent.config.train.offline_training_steps
                - step_count
            )
            left_time = (left_steps * avg_time) / 3600
            batch_queue_size = batch_storage.get_len()
