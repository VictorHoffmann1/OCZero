...
class ReplayBuffer(object):
    ...
    def save_game(self, game, end_tag, gap_steps, priorities=None):
        ...
        if end_tag:
            ...
            valid_len = len(game)
            print(
                f"Saving complete game episode {self._eps_collected} with {valid_len} transitions"
            )
        else:
            valid_len = len(game) - gap_steps
            print(
                f"Saving partial game with {valid_len} valid transitions (gap_steps: {gap_steps})"
            )
        ...
        if priorities is None:
            ...
            assert len(game) == len(priorities), (
