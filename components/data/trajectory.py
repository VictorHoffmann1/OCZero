...
class GameTrajectory:
    ...
    def pad_over(
        ...
        self.bootstrapped_value_lst = self.get_bootstrapped_value(
    ...
    def make_target(self, index):
        ...
        target_search_value = self.search_value_lst[
            index : index + self.unroll_steps + 1
        ]
        target_bt_value = self.bootstrapped_value_lst[
            index : index + self.unroll_steps + 1
        ]
        ...
        assert (
            len(target_reward)
            == len(target_pred_value)
            == len(target_search_value)
            == len(target_policy)
        )
        return (
            np.array(target_obs),
            np.array(target_reward),
            np.array(target_pred_value),
            np.array(target_search_value),
            np.array(target_bt_value),
            np.array(target_policy),
        )
            ...
            for idx in reversed(range(i, i + lens)):
                if idx + td_steps < traj_len:
                    ...
                    bt_value = (
                        0
                        if self.episodic
                        else (self.discount ** (td_steps - steps)) * value_lst[-1]
                    )
                ...
                try:
                    ...
                    delta[cnt] = 0
                advantage[cnt] = (
                    delta[cnt] + self.discount * td_lambda * advantage[cnt + 1]
                )
                cnt -= 1

            value_lst_tmp = np.concatenate(
                (
                    np.asarray(value_lst)[i : i + lens],
                    np.zeros(lens - np.asarray(value_lst)[i : i + lens].shape[0]),
                )
            )
    ...
    def get_current_stacked_obs(self):
        ...
        observations = ray.get(self.obs_lst)[index : index + self.n_stack]
        return observations
    ...
    def get_index_stacked_obs(self, index, padding=False, extra=0):
        ...
        observations = ray.get(self.obs_lst)[
            index : index + self.n_stack + unroll_steps
        ]
        # frames = self.obs_lst[index:index + self.n_stack + unroll_steps]
        if padding:
            pad_len = self.n_stack + unroll_steps - len(observations)
            if pad_len > 0:
                pad_frames = [observations[-1] for _ in range(pad_len)]
                observations = np.concatenate((observations, pad_frames))
        return observations
