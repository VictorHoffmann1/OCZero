...
import numpy as np
import components.model.ctree.cytree as cytree

from torch.nn import L1Loss
from torch.amp import autocast
from components.model.mcts import MCTS
from components.environment.game import GameHistory
from components.utils import select_action
...
class DataWorker(object):
    ...
    def free(self):
        ...
        if self.len_pool() >= self.pool_size:
            print(f"Worker saving {self.len_pool()} game trajectories to replay buffer")
            self.replay_buffer.save_pools.remote(self.trajectory_pool, self.gap_step)
            del self.trajectory_pool[:]

    def put_last_trajectory(
        self, i, last_game_histories, last_game_priorities, game_histories
    ):
        ...
        # pad over and save
        last_game_histories[i].pad_over(
            pad_obs_lst, pad_reward_lst, pad_root_values_lst, pad_child_visits_lst
        )
    ...
    def get_priorities(self, i, pred_values_lst, search_values_lst):
        ...
        if self.config.use_priority and not self.config.use_max_priority:
            pred_values = (
                torch.from_numpy(np.array(pred_values_lst[i])).to(self.device).float()
            )
            search_values = (
                torch.from_numpy(np.array(search_values_lst[i])).to(self.device).float()
            )
            priorities = (
    ...
    def run(self):
        ...
        print(f"Self-play worker {self.rank} starting data collection...")
        envs = [
        ...
        with torch.no_grad():
            while True:
                ...
                for i in range(env_nums):
                    stack_obs_windows[i] = [
                        init_obses[i] for _ in range(self.config.stacked_observations)
                    ]
                ...
                self_play_rewards = 0.0
                self_play_ori_rewards = 0.0
                self_play_moves = 0.0
                self_play_episodes = 0.0
                ...
                while not dones.all() and (step_counter <= self.config.max_moves):
                    ...
                    if (
                        trained_steps
                        >= self.config.training_steps + self.config.last_steps
                    ):
                    ...
                    _temperature = np.array(
                        [
                            self.config.visit_softmax_temperature_fn(
                                num_moves=0, trained_steps=trained_steps
                            )
                            for env in envs
                        ]
                    )
                    ...
                    if new_model_index > self.last_model_index:
                        ...
                        if env_nums > 1:
                            ...
                            if self_play_episodes > 0:
                                log_self_play_moves = (
                                    self_play_moves / self_play_episodes
                                )
                                log_self_play_rewards = (
                                    self_play_rewards / self_play_episodes
                                )
                                log_self_play_ori_rewards = (
                                    self_play_ori_rewards / self_play_episodes
                                )
                            else:
                            ...
                            self.storage.set_data_worker_logs.remote(
                                log_self_play_moves,
                                self_play_moves_max,
                                log_self_play_ori_rewards,
                                log_self_play_rewards,
                                self_play_rewards_max,
                                _temperature.mean(),
                                visit_entropies,
                                0,
                                other_dist,
                    ...
                    for i in range(env_nums):
                        ...
                        if (
                            ...
                            and not self.config.use_max_priority
                            and start_training
                        ):
