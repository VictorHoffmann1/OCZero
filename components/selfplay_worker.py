...
from components.utils import select_action, prepare_observation_lst
            ...
            while True:
                try:
                    ...
                    if (
                        ...
                        print("Training finished, exiting self-play worker...")
                        time.sleep(30)
                        break
                    ...
                    while not dones.all() and (step_counter <= self.config.max_moves):
                        if not start_training:
                            start_training = ray.get(
                                self.storage.get_start_signal.remote()
                            )
                        ...
                        new_model_index = (
                            trained_steps // self.config.checkpoint_interval
                        )
                    ...
                    print(f"Error in self-play worker {self.rank}: {e}")
                    time.sleep(1)
                    continue
