...
import numpy as np
import components.model.ctree.cytree as tree
    ...
    def search(self, roots, model, hidden_state_roots, reward_hidden_roots):
        ...
        with torch.no_grad():
            ...
            for index_simulation in range(self.config.num_simulations):
                ...
                hidden_states = (
                    torch.from_numpy(np.asarray(hidden_states)).to(device).float()
                )
                hidden_states_c_reward = (
                    torch.from_numpy(np.asarray(hidden_states_c_reward))
                    .to(device)
                    .unsqueeze(0)
                )
                hidden_states_h_reward = (
                    torch.from_numpy(np.asarray(hidden_states_h_reward))
                    .to(device)
                    .unsqueeze(0)
                )
