...
from components import agents
from components.utils.format import set_seed, init_logger
from components.worker import start_workers, join_workers
    ...
    try:
        if config.ddp.training_size == 1:
            final_weights, final_model = agent.train(
                rank, replay_buffer_server, storage_server, batch_storage, logger
            )
        else:
            ...
            train_workers = [
                train_ddp.remote(
                    agent,
                    rank * config.ddp.training_size + rank_i,
        ...
        if rank == 0:
            print(f"[main process] master worker failed")
            time.sleep(1)
            join_workers(workers, server_lst)
        raise e
