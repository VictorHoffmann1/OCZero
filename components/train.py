...
from components.log import _log
from components.test import _test
...
def update_weights(
    ...
    obs_batch_ori, action_batch, mask_batch, indices, weights_lst, make_time = (
        inputs_batch
    )
    ...
    if vis_result:
        predicted_values, predicted_policies = (
            scaled_value.detach().cpu(),
            ...
            for step_i in range(config.num_unroll_steps):
                ...
                if vis_result:
                    ...
                    predicted_values = torch.cat(
                        (
                            predicted_values,
                            config.inverse_value_transform(value).detach().cpu(),
                        )
                    )
    ...
    if vis_result:
        ...
        if value_prefix_indices_n1.any():
            other_loss["l1_-1"] = metric_loss(
                predicted_value_prefixs[value_prefix_indices_n1],
                target_value_prefix_base[value_prefix_indices_n1],
            )
        if value_prefix_indices_0.any():
            other_loss["l1_0"] = metric_loss(
                predicted_value_prefixs[value_prefix_indices_0],
                target_value_prefix_base[value_prefix_indices_0],
            )

        td_data = (
            new_priority,
            target_value_prefix.detach().cpu().numpy(),
            target_value.detach().cpu().numpy(),
            transformed_target_value_prefix.detach().cpu().numpy(),
            transformed_target_value.detach().cpu().numpy(),
            target_value_prefix_phi.detach().cpu().numpy(),
            target_value_phi.detach().cpu().numpy(),
            predicted_value_prefixs.detach().cpu().numpy(),
...
def train(config, summary_writer, model_path=None):
    ...
    # reanalyze workers
    print(f"Creating {config.cpu_actor} CPU workers...")
    cpu_workers = [
    ...
    print(f"Creating {config.gpu_actor} GPU workers...")
    gpu_workers = [
    ...
    # self-play workers
    print(f"Creating {config.num_actors} self-play workers...")
    data_workers = [
