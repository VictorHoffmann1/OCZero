...
class MCTS:
    def __init__(self, num_actions, **kwargs):
        ...
        self.visit_num_for_next_phase = (
            max(
                np.floor(
                    self.num_simulations
                    / (np.log2(self.num_top_actions) * self.current_num_top_actions)
                ),
                1,
            )
            * self.current_num_top_actions
        )  # how many visit counts for next phase
        self.used_visit_num = 0
        self.verbose = 0
        assert self.num_top_actions <= self.num_actions

    def search(
        self, model, batch_size, root_states, root_values, root_policy_logits, **kwargs
    ):
                ...
                with torch.no_grad():
                    with autocast(self.device):
                        (
                            ...
                            states, last_actions, reward_hidden
                        )
            ...
            with torch.no_grad():
                with autocast(self.device):
                    (
                        ...
                        current_states, actions[i], reward_hidden
                    )
