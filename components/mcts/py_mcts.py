...
from components.utils.format import softmax
from components.utils.distribution import SquashedNormal
...
class PyMCTS(MCTS):
    ...
    def back_propagate(self, search_path, leaf_node_value, value_min_max):
        ...
        for i in range(path_len - 1, -1, -1):
            ...
            self.log(
                "Update min max value [{:.3f}, {:.3f}] by {:.3f}".format(
                    value_min_max.minimum, value_min_max.maximum, value
                ),
                verbose=3,
            )
