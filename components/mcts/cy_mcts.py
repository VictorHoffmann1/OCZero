...
from components.mcts.ctree import cytree as tree
from components.mcts.ori_ctree import cytree as ori_tree
from components.mcts.ctree_v2 import cytree as tree2
from torch.amp import autocast
from components.utils.format import DiscreteSupport, symexp, pad_and_mask
from components.utils.distribution import SquashedNormal, TruncatedNormal, ContDist
...
class CyMCTS(MCTS):
    ...
    def sample_actions(
        ...
        if input_dist is not None:
            all_actions = torch.cat(
                (all_actions, refined_policy_actions, refined_random_actions), dim=0
            )
        all_actions = all_actions.clip(-0.999, 0.999)

        assert (
            (n_policy + n_random) == sample_nums
            if sample_nums is not None
            else self.num_actions
        )
        ratio = n_policy / (
            sample_nums if sample_nums is not None else self.num_actions
        )
        probs = distr.log_prob(all_actions) - (
            ratio * distr.log_prob(all_actions)
            + (1 - ratio) * random_distr.log_prob(all_actions)
        )
    ...
    def search_continuous(
        ...
        **kwargs,
        ...
        uniform_policy = [
            [0.0 for _ in range(sampled_action_num)] for _ in range(batch_size)
        ]
        leaf_num = 2
        ...
        if use_gumble_noise:
            gumble_noises = np.random.gumbel(
                0, 1, (batch_size, self.num_actions)
            )  # * temperature
        else:
        ...
        roots = tree.Roots(
            batch_size, self.num_actions, self.num_simulations, self.discount
        )
        ...
        value_min_max_lst.set_static_val(
            self.value_minmax_delta, self.c_visit, self.c_scale
        )
        ...
        for simulation_idx in range(self.num_simulations):
            ...
            if self.value_prefix:
                hidden_states_c_reward = torch.stack(hidden_states_c_reward).unsqueeze(
                    0
                )
                hidden_states_h_reward = torch.stack(hidden_states_h_reward).unsqueeze(
                    0
                )
            ...
            leaf_sampled_actions, leaf_policy_priors = self.sample_actions(
                next_logits, sample_nums=leaf_num, add_noise=False
            )
            ...
            if self.value_prefix:
                ...
                reset_idx = np.asarray([1.0 for _ in range(batch_size)])
            ...
            tree.batch_back_propagate(
                ...
                next_value_prefixes.squeeze(-1).tolist(),
                next_values.squeeze(-1).tolist(),
                uniform_policy_non_root,
                value_min_max_lst,
                results,
                to_reset_lst,
            ...
            if self.ready_for_next_gumble_phase(simulation_idx):
                ...
                self.log(
                    "change to phase: {}, top m action -> {}".format(
                        self.current_phase, self.current_num_top_actions
                    ),
                    verbose=3,
                )
        ...
        final_selected_actions = np.asarray(
            [
                root_sampled_actions[i, best_a]
                for i, best_a in enumerate(search_best_actions)
            ]
        )
    ...
    def search_ori_mcts(
        ...
        **kwargs,
        ...
        if use_noise:
            noises = np.asarray(
                [
                    np.random.dirichlet([self.dirichlet_alpha] * self.num_actions)
                    .astype(np.float32)
                    .tolist()
        ...
        if not is_reanalyze:
            search_best_actions = self.select_action(
                search_root_policies,
        ...
        search_root_policies = search_root_policies / search_root_policies.sum(
            -1, keepdims=True
        )
    ...
    def search(
        ...
        **kwargs,
        ...
        roots.prepare(
            root_values.tolist(), root_policy_logits.tolist(), self.num_actions
        )
        # save the min and max value of the tree nodes
        value_min_max_lst = tree.MinMaxStatsList(batch_size)
        value_min_max_lst.set_static_val(
...
class Gumbel_MCTS(object):
            ...
            with torch.no_grad():
                with autocast(self.device):
                    (
                        ...
                        current_states, last_actions, reward_hidden
                    )
    ...
    def run_multi_discrete(
        ...
        reward_sum_pool = [0.0 for _ in range(batch_size)]

        roots = tree2.Roots(batch_size, self.action_space_size, self.num_simulations)
        ...
        reward_hidden_roots = (
            torch.from_numpy(np.zeros((1, batch_size, self.lstm_hidden_size)))
            .float()
            .cuda(),
            torch.from_numpy(np.zeros((1, batch_size, self.lstm_hidden_size)))
            .float()
        ...
        for index_simulation in range(self.num_simulations):
            ...
            (
                hidden_state_index_x_lst,
                hidden_state_index_y_lst,
                last_actions,
                _,
                _,
                _,
            ...
            for ix, iy in zip(hidden_state_index_x_lst, hidden_state_index_y_lst):
                ...
                if self.value_prefix:
                    hidden_states_c_reward.append(
                        reward_hidden_c_pool[ix][0][iy].unsqueeze(0)
                    )
                    hidden_states_h_reward.append(
                        reward_hidden_h_pool[ix][0][iy].unsqueeze(0)
                    )
            ...
            last_actions = (
                torch.from_numpy(np.asarray(last_actions))
                .to("cuda")
                .unsqueeze(1)
                .long()
            )
        ...
        pi_primes = np.asarray(
            roots.get_pi_primes(min_max_stats_lst, c_visit, c_scale, discount)
        )
    ...
    def run_multi_continuous(
        ...
        with torch.no_grad():
            ...
            root_sampled_actions = self.sample_actions(
                root_policy_logits,
                add_noise,
                temperature,
                input_noises,
            ...
            roots = tree2.Roots(batch_size, sampled_action_num, self.num_simulations)
            ...
            assert (
                self.num_top_actions
                == self.config.model.policy_action_num
                + self.config.model.random_action_num
            )
            ...
            gumbels = (
                np.random.gumbel(0, 1, (batch_size, sampled_action_num)) * temperature
            )
            if not use_gumbel_noise:
                gumbels = np.zeros_like(gumbels)
            gumbels = gumbels.tolist()
            ...
            for index_simulation in range(self.num_simulations):
                ...
                if self.value_prefix:
                    hidden_states_c_reward = torch.cat(
                        hidden_states_c_reward, dim=0
                    ).unsqueeze(0)
                    hidden_states_h_reward = torch.cat(
                        hidden_states_h_reward, dim=0
                    ).unsqueeze(0)
                ...
                leaf_sampled_actions = self.sample_actions(
                    policy_logits_pool, False, input_actions=input_actions
                )
                actions_pool.append(leaf_sampled_actions)
                reward_sum_pool = reward_sum_pool.tolist()
                value_pool = value_pool.tolist()
                ...
                if self.value_prefix:
                    ...
                    ]  # TODO: this is a huge bug, previous 0.
        ...
        best_actions = roots.get_actions(
        ...
        return (
            np.asarray(roots.get_values()),
            np.asarray(
                roots.get_pi_primes(min_max_stats_lst, c_visit, c_scale, discount)
            ),
            np.asarray(final_selected_actions),
            min_max_stats_lst.get_min_max(),
            np.asarray(root_sampled_actions),
            np.asarray(best_actions),
        )
