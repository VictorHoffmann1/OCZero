...
from components.worker.watchdog_worker import start_watchdog_server
from components.data.global_storage import GlobalStorage
from components.data.replay_buffer import ReplayBuffer
from components.worker.data_worker import start_data_worker
from components.worker.batch_worker import (
    start_batch_worker,
)
from components.worker.eval_worker import start_eval_worker
from components.utils.format import RayQueue
...
def start_workers(agent, manager, config):
    ...
    storage_server = GlobalStorage.remote(
        agent.build_model(), agent.build_model(), agent.build_model()
    )
    ...
    replay_buffer_server = ReplayBuffer.remote(
    ...
    # data workers
    data_workers = [
        start_data_worker(rank, agent, replay_buffer_server, storage_server, config)
    ...
    batch_workers = [
        start_batch_worker(
            rank, agent, replay_buffer_server, storage_server, batch_storage, config
        )
    ...
    if int(torch.__version__[0]) == 2:
        print(
            f"[main process] torch version is {torch.__version__}, enabled torch_compile."
        )
