...
from components.worker.base import Worker
from components import mcts
from components.environment import make_envs, make_env
...
class DataWorker(Worker):
    ...
    def run(self):
        ...
        envs = make_envs(
            ...
            config.env.game,
            num_envs,
            cur_seed + self.rank * num_envs,
            save_path=video_path,
            episodic_life=config.env.episodic,
            **config.env,
        )  # prev episodic_life=True

        # initialization
        trained_steps = 0  # current training steps
        collected_transitions = ray.get(
            self.replay_buffer.get_transition_num.remote()
        )  # total transitions collected
        start_training = False  # is training
        max_transitions = (
            config.data.total_transitions // config.actors.data_worker
        )  # max transitions to collect in this worker
        dones = [False for _ in range(num_envs)]
        traj_len = [0 for _ in range(num_envs)]

        stack_obs_windows, game_trajs = self.agent.init_envs(
            envs, max_steps=self.config.data.trajectory_size
        )
        prev_game_trajs = [
            None for _ in range(num_envs)
        ]  # previous game trajectories (split a full game trajectory into several sub trajectories)

        # log data
        episode_return = [0.0 for _ in range(num_envs)]
        ...
        while not self.is_finished(trained_steps):
            ...
            with autocast(device_type="cuda"):
                states, values, policies = self.model.initial_inference(
                    current_stacked_obs
                )
            ...
            if (
                collected_transitions % 200 == 0
                and self.config.model.noisy_net
                and self.rank == 0
            ):
                print("*******************************")
                print(
                    f"w_ep={self.model.value_policy_model.pi_net[0].weight_epsilon.mean()}"
                )
                print(
                    f"w_mu={self.model.value_policy_model.pi_net[0].weight_mu.mean()}"
                )
                print(
                    f"w_si={self.model.value_policy_model.pi_net[0].weight_sigma.mean()}"
                )
                print(
                    f"b_ep={self.model.value_policy_model.pi_net[0].bias_epsilon.mean()}"
                )
                print(f"b_mu={self.model.value_policy_model.pi_net[0].bias_mu.mean()}")
                print(
                    f"b_si={self.model.value_policy_model.pi_net[0].bias_sigma.mean()}"
                )
            ...
            for i in range(num_envs):
                ...
                game_trajs[i].store_search_results(
                    values[i], r_values[i], r_policies[i]
                )
                ...
                if game_trajs[i].is_full():
                    ...
                    # new trajectory
                    game_trajs[i] = self.agent.new_game(
                        max_steps=self.config.data.trajectory_size
                    )
                ...
                if dones[i]:
                    ...
                    stacked_obs, traj = self.agent.init_env(
                        envs[i], max_steps=self.config.data.trajectory_size
                    )
        ...
        if padding:
            ...
            if self.config.model.value_target == "bootstrapped":
                ...
                extra = max(
                    0,
                    min(
                        int(1 / (1 - self.config.rl.td_lambda)),
                        self.config.model.GAE_max_steps,
                    )
                    - self.config.rl.unroll_steps
                    - 1,
                )
            ...
            # pad over and save
            prev_game_trajs[idx].pad_over(
                pad_obs_lst,
                pad_reward_lst,
                pad_pred_values_lst,
                pad_search_values_lst,
                pad_policy_lst,
            )
    ...
    def put_trajs(self, traj):
        if self.config.priority.use_priority:
            ...
            if self.config.model.value_target == "bootstrapped":
                target_values = (
                    torch.from_numpy(np.asarray(traj.get_bootstrapped_value()))
                    .cuda()
                    .float()
                )
            elif self.config.model.value_target == "GAE":
                target_values = (
                    torch.from_numpy(np.asarray(traj.get_gae_value())).cuda().float()
                )
            else:
            ...
            priorities = (
                ...
                .cpu()
                .numpy()
                + self.config.priority.min_prior
            )
        else:
            priorities = None
