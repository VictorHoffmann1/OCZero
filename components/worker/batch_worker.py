...
from components import mcts
from components.utils.format import (
    formalize_obs_lst,
    LinearSchedule,
    prepare_obs_lst,
)
...
class BatchWorker(Worker):
    def __init__(self, rank, agent, replay_buffer, storage, batch_storage, config):
        ...
        self.beta_schedule = LinearSchedule(
            self.total_steps, initial_p=config.priority.priority_prob_beta, final_p=1.0
        )
    ...
    def concat_trajs(self, items):
        ...
        for (
            ...
            traj = GameTrajectory(
                ...
                max_size=self.trajectory_size,
                episodic=self.episodic,
                GAE_max_steps=self.GAE_max_steps,
            )
    ...
    def make_batch(self, trained_steps, cnt, real_time=False):
        ...
        for i in range(batch_size):
            ...
            top_new_masks.append(
                int(sample_idx > collected_transitions - self.mixed_value_threshold)
            )

            if self.env in ["DMC", "Gym"]:
                _actions = traj.action_lst[
                    state_index : state_index + self.unroll_steps
                ]
                _unroll_actions = traj.action_lst[
                    state_index + 1 : state_index + 1 + self.unroll_steps
                ]
                # _unroll_actions = traj.action_lst[state_index:state_index + self.unroll_steps]
                _mask = [1.0 for _ in range(_unroll_actions.shape[0])]
                _mask += [0.0 for _ in range(self.unroll_steps - len(_mask))]
                _rand_actions = np.zeros(
                    (self.unroll_steps - _actions.shape[0], self.action_space_size)
                )
                ...
                _mask = [1.0 for _ in range(len(_actions))]
                _mask += [0.0 for _ in range(self.unroll_steps - len(_mask))]
                _actions += [
                    np.random.randint(0, self.action_space_size)
                    for _ in range(self.unroll_steps - len(_actions))
                ]
        ...
        obs_lst = prepare_obs_lst(obs_lst)
        inputs_batch = [
            obs_lst,
            action_lst,
            mask_lst,
        ...
        if batch_size - reanalyze_batch_size > 0:
            batch_policies_non_re = self.prepare_policy_non_reanalyze(
                traj_lst[reanalyze_batch_size:],
                transition_pos_lst[reanalyze_batch_size:],
            )
        else:
        ...
        if self.env in ["DMC", "Gym"]:
            batch_best_actions = best_actions.reshape(
                batch_size, self.unroll_steps + 1, self.action_space_size
            )
        else:
            batch_best_actions = np.asarray(best_actions).reshape(
        ...
        # log
        self.storage.add_log_scalar.remote({"batch_worker/td_step": np.mean(td_steps)})
    ...
    # @profile
    def prepare_reward_value_gae_faster(
        self,
        traj_lst,
        transition_pos_lst,
        indices_lst,
        ...
        extra = max(
            0,
            min(int(1 / (1 - self.td_lambda)), self.GAE_max_steps)
            - self.unroll_steps
            - 1,
        )

        # init
        value_obs_lst, td_steps_lst, value_mask, policy_mask = (
            [],
            [],
            [],
            [],
        ...
        for traj, state_index, idx in zip(traj_lst, transition_pos_lst, indices_lst):
            ...
            for current_index in range(
                state_index, state_index + self.unroll_steps + 1 + extra + td_steps
            ):
        ...
        state_lst, value_lst, policy_lst = self.efficient_inference(
            value_obs_lst, only_value=False
        )
        # v_{t+k}
        batch_size = len(value_lst)
        value_lst = value_lst.reshape(-1) * (
            np.array([self.discount for _ in range(batch_size)]) ** td_steps_lst
        )
        ...
        for traj, state_index, idx in zip(traj_lst, transition_pos_lst, indices_lst):
            ...
            for current_index in reversed(
                range(state_index, state_index + self.unroll_steps + 1 + extra)
            ):
                bootstrap_index = current_index + td_steps_lst[value_index + index]
                for i, reward in enumerate(
                    traj.reward_lst[current_index:bootstrap_index]
                ):
                    td_value_lst[value_index + index + td_steps] += (
                        reward * self.discount**i
                    )

                delta[index] = (
                    td_value_lst[value_index + index + td_steps]
                    - value_lst[value_index + index]
                )
                advantage[index] = (
                    delta[index] + self.discount * td_lambda * advantage[index + 1]
                )
                index -= 1

            target_values_tmp = (
                advantage[: self.unroll_steps + 1]
                + np.asarray(value_lst)[
                    value_index : value_index + self.unroll_steps + 1
                ]
            )
            ...
            for i, current_index in enumerate(
                range(state_index, state_index + self.unroll_steps + 1)
            ):
        ...
        if self.rank == 0 and self.cnt % 20 == 0:
            print(
                f"--------------- lambda={np.asarray(td_lambdas).mean():.3f} -------------------"
            )
        ...
        for i, idx in enumerate(indices_lst):
            value_masks.append(
                int(idx > collected_transitions - self.mixed_value_threshold)
            )
            value_index += self.unroll_steps + 1 + extra

        value_masks = np.asarray(value_masks)
        return (
            np.asarray(batch_value_prefixes),
            np.asarray(batch_values),
            np.asarray(td_steps_lst).flatten(),
            (re_state_lst, re_value_lst, re_policy_lst, re_policy_mask),
            value_masks,
        )

    # @profile
    def prepare_reward_value_gae(
        self,
        traj_lst,
        transition_pos_lst,
        indices_lst,
        ...
        for traj, state_index, idx in zip(traj_lst, transition_pos_lst, indices_lst):
            ...
            for current_index in range(
                state_index, state_index + self.unroll_steps + 1 + extra
            ):
        ...
        state_lst, ori_cur_value_lst, policy_lst = self.efficient_inference(
            policy_obs_lst, only_value=False
        )
        # v_{t+k}
        batch_size = len(value_lst)
        value_lst = value_lst.reshape(-1) * (
        ...
        for traj, state_index, idx in zip(traj_lst, transition_pos_lst, indices_lst):
            ...
            for current_index in reversed(
                ...
                delta[index] = (
                    value_lst[value_index + index] - cur_value_lst[value_index + index]
                )
                advantage[index] = (
                    delta[index] + self.discount * td_lambda * advantage[index + 1]
                )
    ...
    def prepare_reward(
        ...
        for traj, state_index, idx in zip(traj_lst, transition_pos_lst, indices_lst):
            ...
            horizon_id = 0
            value_prefix = 0.0
            top_value_masks.append(
                int(
                    idx
                    > collected_transitions
                    - self.config.train.start_use_mix_training_steps
                )
            )
            for current_index in range(
                state_index, state_index + self.unroll_steps + 1
            ):
                # reset every lstm_horizon_len
                if horizon_id % self.lstm_horizon_len == 0 and self.value_prefix:
                    value_prefix = 0.0
        ...
        for traj, state_index, ind in zip(traj_lst, transition_pos_lst, indices_lst):
            ...
            for current_index in range(
                ...
                if policy_mask[policy_index]:
                    ...
                    if (
                        r_policies[policy_index].argmax()
                        != search_best_indexes[policy_index]
                    ):
                    ...
                    if self.env in ["DMC", "Gym"]:
                        target_policies.append(
                            [0 for _ in range(sampled_actions.shape[1])]
                        )
                    else:
                        target_policies.append(
                            [0 for _ in range(self.action_space_size)]
                        )
    ...
    @torch.no_grad()
    def imagine_episodes(
        self, pre_lst, traj_lst, transition_pos_lst, trained_steps, policy="search"
    ):
        length = 1
        times = 3
        ...
        reward_lst = []
        value_lst = []
        temperature = self.agent.get_temperature(trained_steps=trained_steps) * np.ones(
            (len(states), 1)
        )
        for i in range(length):
            if policy == "search":
                tree = mcts.names[self.config.mcts.language](
                    num_actions=self.config.env.action_space_size
                    if self.env == "Atari"
                    else self.config.mcts.num_top_actions,
                    discount=self.config.rl.discount,
                    **self.config.mcts,  # pass mcts related params
                    **self.config.model,  # pass the value and reward support params
                )
                if self.env == "Atari":
                    ...
                    r_values, r_policies, best_actions, sampled_actions, _, _ = (
                        tree.search_continuous(
                            self.model,
                            len(states),
                            states,
            ...
            if self.value_prefix and (i + 1) % self.lstm_horizon_len == 0:
                reward_hidden = (
                    torch.zeros(
                        1, len(states), self.config.model.lstm_hidden_size
                    ).cuda(),
                    torch.zeros(
                        1, len(states), self.config.model.lstm_hidden_size
                    ).cuda(),
                ...
                true_rewards = (
                    value_prefixes.squeeze().detach().cpu().numpy()
                    - last_values_prefixes
                )
    ...
    def efficient_inference(self, obs_lst, only_value=False, value_idx=0):
        ...
        with torch.no_grad():
            for i in range(slices):
                ...
                current_obs = formalize_obs_lst(current_obs)
...
def start_batch_worker(rank, agent, replay_buffer, storage, batch_storage, config):
    ...
    worker = BatchWorker.remote(
        rank, agent, replay_buffer, storage, batch_storage, config
    )
    print(
        f"[Batch worker GPU] Starting batch worker GPU {rank} at process {os.getpid()}."
    )
    worker.run.remote()


def start_batch_worker_cpu(
    rank, agent, replay_buffer, storage, prebatch_storage, config
):
    worker = BatchWorker_CPU.remote(
        rank, agent, replay_buffer, storage, prebatch_storage, config
    )
    print(
        f"[Batch worker CPU] Starting batch worker CPU {rank} at process {os.getpid()}."
    )
    worker.run.remote()


def start_batch_worker_gpu(
    rank, agent, replay_buffer, storage, prebatch_storage, batch_storage, config
):
    worker = BatchWorker_GPU.remote(
        rank, agent, replay_buffer, storage, prebatch_storage, batch_storage, config
    )
    print(
        f"[Batch worker GPU] Starting batch worker GPU {rank} at process {os.getpid()}."
